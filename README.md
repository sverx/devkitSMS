...
* build the final .sms/.gg file with the makesms utility included (the input file is the ihx generated by SDCC linker, the output file is the generated ROM). For example:
```
...
* put your data into a separate .c file for each 16 KiB ROM bank starting from bank2, for example bank2.c, bank3.c etc... (you can use the assets2banks tool described above to achieve this easily) compiling each one with a different CONST segment name, I suggest using `BANK#` for descriptiveness:
```
...
* put your data into a separate .c file for each 16 KiB ROM bank starting from bank1, for example bank1.c, bank2.c etc... (you can use the assets2banks described above to achieve this easily) compiling each one with a different CONST segment name, I suggest using `BANK#` for descriptiveness. Note that only 16,320 bytes (16 KiB - 64 bytes) are available in each ROM bank.
```
